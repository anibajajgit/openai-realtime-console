...
export default function App() {
  ...
  // Log the mount of the App component
  useEffect(() => {
    console.log("App component mounted with user:", user);
    
    // Initialize call sound
    callSoundRef.current = new Audio('/assets/call-sound.mp3');
  ...
  // Set selected roles and scenarios from localStorage if available
  useEffect(() => {
    const roleFromStorage = localStorage.getItem('selectedRole');
    const scenarioFromStorage = localStorage.getItem('selectedScenario');

    if (roleFromStorage) {
      setSelectedRole(JSON.parse(roleFromStorage));
      console.log("Selected role:", JSON.parse(roleFromStorage));
    }

    if (scenarioFromStorage) {
      setSelectedScenario(JSON.parse(scenarioFromStorage));
    }
  }, []);
    ...
    if (dataChannel) {
      dataChannel.addEventListener("message", (e) => {
        try {
          ...
          setEvents((prev) => {
            // For text response completion events, update the events array immediately
            if (event.type === "response.text.done" || 
                event.type === "response.audio_transcript.done" || 
                event.type === "conversation.item.input_audio_transcription.completed") {
              return [event, ...prev];
            }
            // For other event types, log them but don't add to events array
            if (event.type !== "response.text.delta" && 
                event.type !== "response.audio_transcript.delta" && 
                event.type !== "input_audio_buffer") {
              console.log("Non-transcription event:", event.type);
            }
            return prev;
          });
        ...
        <Route path="/scenarios" element={
          user ? (
            <>
              <nav className="absolute top-0 left-0 right-0 h-16 flex items-center">
                <div className="flex items-center gap-4 w-full m-4 pb-2 border-0 border-b border-solid border-gray-200">
                  <h1>Practice your scenario</h1>
                </div>
              </nav>
              <main className="fixed top-16 left-0 right-0 bottom-0 overflow-auto md:overflow-hidden">
                <div className="flex flex-col md:flex-row h-full bg-gray-50">
                  <AppSidebar />
                  <section className="w-full p-4 flex flex-col gap-4 md:flex-row md:overflow-auto relative">
                    {/* Overlay div that appears when session is active */}
                    {isSessionActive && (
                      <div className="absolute inset-0 bg-gray-800/50 backdrop-blur-sm z-10"></div>
                    )}
                    <div className="w-full md:w-1/3 h-auto md:h-full overflow-auto z-20">
                      <ScenarioSelector />
                      <EventLog events={events} />
                    </div>
                    <div className={`flex flex-col gap-6 w-full md:w-2/3 h-auto md:h-full ${isSessionActive ? 'z-20 ring-4 ring-blue-500 ring-opacity-75 rounded-xl' : ''}`}>
                      <div className="w-5/6 h-32 md:h-48 mr-auto">
                        {selectedRole ? (
                          <img
                            src={selectedRole.photoUrl}
                            alt={selectedRole.name}
                            className="h-full aspect-square object-cover rounded-lg shadow-md"
                            onError={(e) => {
